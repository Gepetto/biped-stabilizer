...
stepping_center = np.array(
    [0, 2 * foot_corner[1] + stepping_corner[1] + feet_sep]
)
...
LF_translation = np.array([0.0, strt_y, 0]).T
...
regular_step_duration = round(
    ss_duration + ds_duration, 5
)  # number of nominal periods for step
step_samples = int(round(regular_step_duration / mpc_period))
horizon_lenght = (
    num_steps * step_samples
)  # number of iterations in preview horizon
...
#    int_gainXY = np.array([0.5, 0])
    ...
    centGain = np.array(
        [[-4719.669152989827, -2040.1393274991938, -194.82405448186182]]
    )
