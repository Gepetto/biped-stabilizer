...
void setPoseTranslation(Eigen::Isometry3d& self, const Eigen::Vector3d& translation)
{
  self.translation() = translation;
}

void setPoseRotationQuaternion(Eigen::Isometry3d& self, const Eigen::Quaterniond& quat)
{
...
void setPoseRotationMatrix(Eigen::Isometry3d& self, const Eigen::Matrix3d& rot)
{
  const Eigen::Vector3d trans = self.translation();
  self = rot;
  self.translation() = trans;
...
std::ostream& wrapperDisplayPose(std::ostream& os, const Eigen::Isometry3d& pose)
{
  os << "Translation : " << pose.translation().transpose() << std::endl
     << "Rotation : " << std::endl
     << pose.rotation().matrix();
...
void printPose(Eigen::Isometry3d& self)
{
...
void exposeIsometry3d()
{
  boost::python::class_<Eigen::Isometry3d>("eMatrixHom")
      ...
      .def("set_translation", &setPoseTranslation,
           boost::python::args("self", "translation"), "set the translation as a vector 3d")
      .def("set_rotation", &setPoseRotationQuaternion,
           boost::python::args("self", "quaternion"), "set the rotation from a Quaternion")
      .def("set_rotation", &setPoseRotationMatrix,
           boost::python::args("self", "rotation"), "set the rotation from a 3d matrix");
}
