...
template <typename T, std::size_t M, std::size_t N>
inline void matrix<T, M, N>::swap(const unsigned int& x1,
...
template <typename T>
inline void eigenvalues(const matrix<T, 2, 2>& matrix, T& eigenvalue1,
                        T& eigenvalue2) {
  T delta = sqrt<T>(sqr<T>(matrix(0, 0) - matrix(1, 1)) +
                    T(4.0) * matrix(1, 0) * matrix(0, 1));
...
inline void eigenvector(const matrix<T, 2, 2>& matrix,
  ...
  eigenvector1 = normalize(
      make_vector(T(-1.0) * matrix(1, 0), matrix(0, 0) - eigenvalue1));
  eigenvector2 = normalize(
      make_vector(T(-1.0) * matrix(1, 0), matrix(0, 0) - eigenvalue2));
}
