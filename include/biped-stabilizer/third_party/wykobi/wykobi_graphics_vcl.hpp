...
class wykobi_graphics_vcl {
 public:
  wykobi_graphics_vcl(TCanvas* c, unsigned int w, unsigned int h)
      : canvas(c), _width(w), _height(h), color(clBlack) {
  ...
  inline void draw_circle(const T x, const T& y, const T& radius) const {
    canvas->Ellipse(static_cast<int>(x - radius), static_cast<int>(y - radius),
  ...
  inline void draw_line(const point2d<T>& point1,
                        const point2d<T>& point2) const {
