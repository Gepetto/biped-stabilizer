...
class wykobi_graphics_opengl {
 public:
  wykobi_graphics_opengl(const unsigned int& w, const unsigned int& h,
                         DrawingMode dm)
      : _width(w),
        _height(h),
        drawing_mode(dm),
  ...
  inline void set_color(const unsigned int& _red, const unsigned int& _green,
  ...
  inline void draw_segment(const T& x1, const T& y1, const T& z1, const T& x2,
                           const T& y2, const T& z2) const {
    glBegin(GL_LINES);
    glVertex3d(x1, y1, z1);
    glVertex3d(x2, y2, z2);
    glEnd();
  ...
  inline void draw_triangle(const T& x1, const T& y1, const T& z1, const T& x2,
  ...
  inline void draw_quadix(const T& x1, const T& y1, const T& z1, const T& x2,
                          const T& y2, const T& z2, const T& x3, const T& y3,
                          const T& z3, const T& x4, const T& y4,
                          const T& z4) const {
    switch (drawing_mode) {
      case eSolid:
        glBegin(GL_QUADS);
  ...
  inline void draw_circle(const T& x, const T& y, const T& radius) const {
    ...
    for (std::size_t i = (drawing_mode == eSolid) ? 1 : 0; i < _polygon.size();
         ++i) {
  ...
  inline void draw_triangle(const point2d<T>& point1, const point2d<T>& point2,
                            const point2d<T>& point3) const {
    draw_triangle(point1.x, point1.y, point2.x, point2.y, point3.x, point3.y);
  }

  inline void draw_triangle(const point3d<T>& point1, const point3d<T>& point2,
                            const point3d<T>& point3) const {
    draw_triangle(point1.x, point1.y, point1.z, point2.x, point2.y, point2.z,
                  point3.x, point3.y, point3.z);
  }

  inline void draw_quadix(const point2d<T>& point1, const point2d<T>& point2,
                          const point2d<T>& point3,
                          const point2d<T>& point4) const {
    draw_quadix(point1.x, point1.y, point2.x, point2.y, point3.x, point3.y,
                point4.x, point4.y);
  }

  inline void draw_quadix(const point3d<T>& point1, const point3d<T>& point2,
                          const point3d<T>& point3,
                          const point3d<T>& point4) const {
    draw_quadix(point1.x, point1.y, point1.z, point2.x, point2.y, point2.z,
                point3.x, point3.y, point3.z, point4.x, point4.y, point4.z);
  }

  inline void draw_rectangle(const point2d<T>& point1,
                             const point2d<T>& point2) const {
  ...
  inline void draw(const rectangle<T>& rectangle) {
    draw_rectangle(rectangle[0], rectangle[1]);
  }
  inline void draw(const quadix<T, 2>& quadix) {
    draw_quadix(quadix[0], quadix[1], quadix[2], quadix[3]);
  }
  ...
  inline void draw(const segment<T, 3>& segment) {
    draw_segment(segment[0], segment[1]);
  }
  inline void draw(const triangle<T, 3>& triangle) {
    draw_triangle(triangle[0], triangle[1], triangle[2]);
  }
  inline void draw(const quadix<T, 3>& quadix) {
    draw_quadix(quadix[0], quadix[1], quadix[2], quadix[3]);
  }
  inline void draw(const sphere<T>& sphere) {
    draw_sphere(sphere.x, sphere.y, sphere.radius);
  }
  ...
  inline void draw(const cubic_bezier<T, 3>& bezier,
                   const std::size_t& point_count) {
    std::vector<point2d<T> > point_list;
    wykobi::generate_bezier(bezier, point_count, point_list);
    draw_polyline(point_list);
  ...
  inline void draw(const quadratic_bezier<T, 3>& bezier,
                   const std::size_t& point_count) {
    std::vector<point2d<T> > point_list;
    wykobi::generate_bezier(bezier, point_count, point_list);
    draw_polyline(point_list);
