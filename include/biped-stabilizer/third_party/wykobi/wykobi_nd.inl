...
inline bool parallel(const line<T, D>& line1, const line<T, D>& line2) {
  ...
  return is_equal(sqr(dot_product(v1, v2)),
                  dot_product(v1, v1) * dot_product(v2, v2));
}

template <typename T, std::size_t D>
inline bool parallel(const segment<T, D>& segment1,
...
inline bool perpendicular(const line<T, D>& line1, const line<T, D>& line2) {
  return is_equal(dot_product(line1[0] - line1[1], line2[0] - line2[1]),
                  T(0.0));
}

template <typename T, std::size_t D>
inline bool perpendicular(const segment<T, D>& segment1,
                          const segment<T, D>& segment2) {
  return is_equal(
      dot_product(segment1[0] - segment1[1], segment2[0] - segment2[1]),
      T(0.0));
}

template <typename T, std::size_t D>
inline bool collinear(const pointnd<T, D>& point1, const pointnd<T, D>& point2,
...
template <typename T, std::size_t D>
inline bool robust_collinear(const pointnd<T, D>& point1,
                             const pointnd<T, D>& point2,
                             const pointnd<T, D>& point3) {
  return (less_than_or_equal(
      lay_distance(
          closest_point_on_line_from_point(make_line(point1, point2), point3),
          point3),
      T(0.0)));
}

template <typename T, std::size_t D>
inline bool is_point_collinear(const segment<T, D>& segment,
                               const pointnd<T, D>& point, const bool robust) {
  ...
  if (robust)
    return robust_collinear(pointnd<T, D>(segment[0]),
                            pointnd<T, D>(segment[1]), point);
  else
    return collinear(pointnd<T, D>(segment[0]), pointnd<T, D>(segment[1]),
                     point);
}

template <typename T, std::size_t D>
inline bool intersect(const segment<T, D>& segment1,
                      const segment<T, D>& segment2, const T& fuzzy) {
  return is_equal(distance(segment1, segment2), fuzzy);
}

template <typename T, std::size_t D>
inline bool intersect(const line<T, D>& line1, const line<T, D>& line2,
                      const T& fuzzy) {
  return is_equal(distance(line1, line2), fuzzy);
}

template <typename T, std::size_t D>
inline pointnd<T, D> intersection_point(const segment<T, D>& segment1,
                                        const segment<T, D>& segment2,
                                        const T& fuzzy) {
...
template <typename T, std::size_t D>
inline pointnd<T, D> intersection_point(const line<T, D>& line1,
...
template <typename T, std::size_t D>
inline T distance(const segment<T, D>& segment1,
                  const segment<T, D>& segment2) {
...
template <typename T, std::size_t D>
inline T lay_distance(const pointnd<T, D>& point1,
                      const pointnd<T, D>& point2) {
  T sum = T(0.0);
...
template <typename T, std::size_t D>
inline T lay_distance(const pointnd<T, D>& point,
                      const segment<T, D>& segment) {
  return lay_distance(closest_point_on_segment_from_point(segment, point),
                      point);
}
...
template <typename T, std::size_t D>
inline T lay_distance(const segment<T, D>& segment1,
                      const segment<T, D>& segment2) {
  const vectornd<T, D> u = segment1[1] - segment1[0];
  const vectornd<T, D> v = segment2[1] - segment2[0];
  const vectornd<T, D> w = segment1[0] - segment2[0];
...
template <typename T, std::size_t D>
inline T manhattan_distance(const pointnd<T, D>& point1,
                            const pointnd<T, D>& point2) {
  T sum = T(0.0);
...
template <typename T, std::size_t D>
inline T chebyshev_distance(const pointnd<T, D>& point1,
                            const pointnd<T, D>& point2) {
...
template <typename T, std::size_t D>
inline T inverse_chebyshev_distance(const pointnd<T, D>& point1,
                                    const pointnd<T, D>& point2) {
...
template <typename T, std::size_t D>
inline bool point_in_sphere(const pointnd<T, D>& point,
                            const hypersphere<T, D>& sphere) {
  return less_than_or_equal(sqr(sphere.radius),
                            lay_distance(point, pointnd<T, D>(sphere.center)));
}

template <typename T, std::size_t D>
inline pointnd<T, D> closest_point_on_segment_from_point(
    const segment<T, D>& segment, const pointnd<T, D>& point) {
...
template <typename T, std::size_t D>
inline pointnd<T, D> closest_point_on_line_from_point(
    const line<T, D>& line, const pointnd<T, D>& point) {
...
template <typename T, std::size_t D>
inline pointnd<T, D> closest_point_on_sphere_from_point(
    const hypersphere<T, D>& sphere, const pointnd<T, D>& point) {
  return sphere.center + sphere.radius / distance(sphere.center, point) *
                             (point - sphere.center);
}

template <typename T, std::size_t D>
inline pointnd<T, D> closest_point_on_plane_from_point(
    const plane<T, D>& plane, const pointnd<T, D>& point) {
...
template <typename T, std::size_t D>
inline pointnd<T, D> closest_point_on_box_from_point(
    const box<T, D>& box, const pointnd<T, D>& point) {
...
template <typename T, std::size_t D>
inline pointnd<T, D> mirror(const pointnd<T, D>& point,
                            const line<T, D>& mirror_axis) {
  return project_point_t(
      point, closest_point_on_line_from_point(mirror_axis, point), T(2.0));
}

template <typename T, std::size_t D>
inline segment<T, D> mirror(const segment<T, D>& segment,
                            const line<T, D>& mirror_axis) {
...
template <typename T, std::size_t D>
inline line<T, D> mirror(const line<T, D>& line,
...
template <typename T, std::size_t D>
inline triangle<T, D> mirror(const triangle<T, D>& triangle,
                             const line<T, D>& mirror_axis) {
...
template <typename T, std::size_t D>
inline quadix<T, D> mirror(const quadix<T, D>& quadix,
                           const line<T, D>& mirror_axis) {
...
template <typename T, std::size_t D>
inline hypersphere<T, D> mirror(const hypersphere<T, D>& sphere,
                                const line<T, D>& mirror_axis) {
...
template <typename T, std::size_t D>
inline polygon<T, D> mirror(const polygon<T, D>& polygon,
                            const line<T, D>& mirror_axis) {
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const pointnd<T, D>& point,
                                       const line<T, D>& axis) {
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const triangle<T, D>& triangle,
                                       const line<T, D>& axis) {
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const box<T, D>& box,
                                       const line<T, D>& axis) {
  std::vector<pointnd<T, D> > point_list;
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const quadix<T, D>& quadix,
                                       const line<T, D>& axis) {
  std::vector<pointnd<T, D> > point_list;
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const hypersphere<T, D>& sphere,
                                       const line<T, D>& axis) {
  ...
  point_list.push_back(closest_point_on_line_from_point(
      axis, point_list.front() + (v * sphere.radius)));
  point_list.push_back(closest_point_on_line_from_point(
      axis, point_list.front() - (v * sphere.radius)));
...
template <typename T, std::size_t D>
inline segment<T, D> project_onto_axis(const polygon<T, D>& polygon,
                                       const line<T, D>& axis) {
  if (polygon.size() == 0) return degenerate_segmentnd<T, D>();
...
inline pointnd<T, D> generate_random_point(const quadix<T, D>& quadix) {
  ...
  return ((r1 * quadix[0]) + (r2 * quadix[1]) + (r3 * quadix[2]) +
          (r4 * quadix[3])) *
         T(0.25);
}
...
inline pointnd<T, D> generate_random_point(const box<T, D>& box) {
  ...
  for (std::size_t i = 0; i < D; ++i) {
    _point[i] = min(box[0][i], box[1][i]) +
                generate_random_value(abs(box[0][i] - box[1][i]));
  }
...
template <typename T, std::size_t D>
inline void generate_random_points(const box<T, D>& box,
                                   std::vector<pointnd<T, D> >& point_list) {
...
template <typename T, std::size_t D>
inline void generate_random_points(const segment<T, D>& segment,
                                   std::vector<pointnd<T, D> >& point_list) {
  for (std::size_t i = 0; i < point_list.size(); ++i) {
...
template <typename T, std::size_t D>
inline void generate_random_points(const triangle<T, D>& triangle,
                                   std::vector<pointnd<T, D> >& point_list) {
  for (std::size_t i = 0; i < point_list.size(); ++i) {
...
template <typename T, std::size_t D>
inline void generate_random_points(const quadix<T, D>& quadix,
                                   std::vector<pointnd<T, D> >& point_list) {
  for (std::size_t i = 0; i < point_list.size(); ++i) {
...
template <typename T, std::size_t D>
inline vectornd<T, D> operator+(const vectornd<T, D>& v1,
...
template <typename T, std::size_t D>
inline pointnd<T, D> operator+(const vectornd<T, D>& v,
...
template <typename T, std::size_t D>
inline vectornd<T, D> operator-(const pointnd<T, D>& p1,
...
template <typename T>
inline vectornd<T, 3> operator*(const vectornd<T, 3>& v1,
...
template <typename T, std::size_t D>
inline pointnd<T, D> operator+(const pointnd<T, D>& p1,
                               const pointnd<T, D>& p2) {
  pointnd<T, D> _point;
...
template <typename T, std::size_t D>
inline bool operator<(const pointnd<T, D>& point1,
                      const pointnd<T, D>& point2) {
  for (std::size_t i = 0; i < D; ++i) {
...
template <typename T, std::size_t D>
inline bool not_equal(const pointnd<T, D>& point1,
                      const pointnd<T, D>& point2) {
  return !is_equal(point1, point2);
}
...
inline triangle<T, D> degenerate_trianglend() {
  return make_triangle(degenerate_pointnd<T, D>(), degenerate_pointnd<T, D>(),
                       degenerate_pointnd<T, D>());
}

template <typename T, std::size_t D>
inline quadix<T, D> degenerate_quadixnd() {
  return make_quadix(degenerate_pointnd<T, D>(), degenerate_pointnd<T, D>(),
                     degenerate_pointnd<T, D>(), degenerate_pointnd<T, D>());
}
...
template <typename T, std::size_t D>
inline ray<T, D> make_ray(const pointnd<T, D>& origin,
...
template <typename T, std::size_t D>
inline segment<T, D> make_segment(const pointnd<T, D>& point1,
                                  const pointnd<T, D>& point2) {
...
template <typename T, std::size_t D>
inline line<T, D> make_line(const pointnd<T, D>& point1,
                            const pointnd<T, D>& point2) {
  line<T, D> _line;
...
template <typename T, std::size_t D>
inline box<T, D> make_box(const pointnd<T, D>& point1,
                          const pointnd<T, D>& point2) {
  box<T, D> _box;
...
template <typename T, std::size_t D>
inline triangle<T, D> make_triangle(const pointnd<T, D>& point1,
                                    const pointnd<T, D>& point2,
                                    const pointnd<T, D>& point3) {
...
template <typename T, std::size_t D>
inline quadix<T, D> make_quadix(const pointnd<T, D>& point1,
                                const pointnd<T, D>& point2,
                                const pointnd<T, D>& point3,
...
template <typename T, std::size_t D>
inline hypersphere<T, D> make_sphere(const pointnd<T, D>& point,
                                     const T& radius) {
...
template <typename T, std::size_t D>
inline hypersphere<T, D> make_sphere(const pointnd<T, D>& point1,
                                     const pointnd<T, D>& point2) {
  hypersphere<T, D> sphere;
...
template <typename T, std::size_t D>
inline polygon<T, D> make_polygon(
