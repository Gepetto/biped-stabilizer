...
struct sutherland_hodgman_polygon_clipper<point2d<T> > {
 public:
  sutherland_hodgman_polygon_clipper(const rectangle<T>& clip_boundry,
    ...
    clipper_engine.register_edge(edge(clip_boundry, 0)[1],
                                 edge(clip_boundry, 0)[0]);
    clipper_engine.register_edge(edge(clip_boundry, 1)[1],
                                 edge(clip_boundry, 1)[0]);
    clipper_engine.register_edge(edge(clip_boundry, 2)[1],
                                 edge(clip_boundry, 2)[0]);
    clipper_engine.register_edge(edge(clip_boundry, 3)[1],
                                 edge(clip_boundry, 3)[0]);

    clipped_polygon.clear();

    clipper_engine.clip(input_polygon.begin(), input_polygon.end(),
  ...
  sutherland_hodgman_polygon_clipper(const ClipObject& clip_boundry,
    ...
    if (orientation(clip_boundry[0], clip_boundry[1], clip_boundry[2]) ==
        LeftHandSide) {
      std::size_t j = 0;

      for (std::size_t i = clip_boundry.size() - 1;
           (0 <= i) && (i < clip_boundry.size()); i--) {
...
struct sutherland_hodgman_polygon_clipper_engine<point2d<T> > {
 public:
  void register_edge(const point2d<T>& point1, const point2d<T>& point2) {
    edge_list.push_back(
        std::make_pair /*<const point2d<T>, const point2d<T> >*/ (point1,
                                                                  point2));
  }
  ...
  void clip(InputIterator begin, InputIterator end, OutputIterator out) {
    ...
    for (std::size_t i = 0; i < edge_list.size(); ++i) {
      ...
      switch (i & 0x01) {
        case 0:
          clip_against_edge(half_plane_edge(point1, point2), clip_poly1,
                            clip_poly2);
          break;

        case 1:
          clip_against_edge(half_plane_edge(point1, point2), clip_poly2,
                            clip_poly1);
          break;
      }
    }
    ...
    switch (edge_list.size() & 0x01) {
      case 0:
        std::copy(clip_poly1.begin(), clip_poly1.end(), out);
        break;

      case 1:
        std::copy(clip_poly2.begin(), clip_poly2.end(), out);
        break;
    }
  }
 ...
 private:
  ...
  void clip_against_edge(const half_plane_edge& edge,
    ...
    for (typename std::vector<point2d<T> >::iterator it = input_poly.begin();
         it != input_poly.end(); ++it) {
      ...
      if (current_point_in && previous_point_in) {
        ...
        clipped_poly.push_back(
            edge.intersection_point(current_point, previous_point));
      } else if (current_point_in && !previous_point_in) {
        clipped_poly.push_back(
            edge.intersection_point(previous_point, current_point));
        clipped_poly.push_back(current_point);
      }
