...
enum eDisplayMode { eDisplayMode2D, eDisplayMode3D };
...
class wykobi_window {
 public:
  wykobi_window(int argc, char* argv[]) {
    ...
    if (handle->display_mode == eDisplayMode2D) {
      ...
      gluOrtho2D(0.0, handle->window_width() - 1, handle->window_height() - 1,
                 0.0);
 ...
 private:
  ...
  static void glut_keyboard_handler(unsigned char key, int x, int y) {
    handle->keyboard_handler(key, x, y);
  }
  static void glut_special_key_handler(int key, int x, int y) {
    handle->special_key_handler(key, x, y);
  }
  static void glut_reshape_handler(int width, int height) {
    handle->reshape_handler(width, height);
  }
  static void glut_mouse_handler(int button, int state, int x, int y) {
    handle->mouse_handler(button, state, x, y);
  }
  static void glut_motion_handler(int x, int y) {
    handle->motion_handler(x, y);
  }
  static void glut_menu_handler(int menu_choice) {
    handle->menu_handler(menu_choice);
  }
  static void glut_timmer_handler(int ms_time) {
    handle->timmer_handler(ms_time);
  }
