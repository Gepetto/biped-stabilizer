...
struct convex_hull_graham_scan<point2d<T> > {
 public:
  ...
  convex_hull_graham_scan(InputIterator begin, InputIterator end,
                          OutputIterator out) {
    ...
    for (typename std::vector<gs_point>::iterator it = ++point.begin();
         it != point.end(); ++it) {
    ...
    while (it != point.end()) {
      if (pnt_queue.size() > 1) {
        if (orientation((*++pnt_queue.begin()), (*pnt_queue.begin()), (*it)) ==
            CounterClockwise)
    ...
    for (typename std::deque<gs_point>::iterator itr = pnt_queue.begin();
         itr != pnt_queue.end(); ++itr) {
 ...
 private:
  class gs_point : public point2d<T> {
   public:
    gs_point(const T& _x = T(0.0), const T& _y = T(0.0), const T& _ang = T(0.0))
        : angle(_ang) {
      point2d<T>::x = _x;
      point2d<T>::y = _y;
    }
...
struct convex_hull_jarvis_march<point2d<T> > {
 public:
  template <typename InputIterator, typename OutputIterator>
  convex_hull_jarvis_march(InputIterator begin, InputIterator end,
                           OutputIterator out) {
    ...
    do {
      current_point =
          (not_equal(previous_point, *begin)) ? *begin : *(begin + 1);
    ...
    if (orientation(*previous_it, point_list.back(), point_list.front()) !=
...
struct convex_hull_melkman<point2d<Type> > {
 public:
  template <typename InputIterator, typename OutputIterator>
  convex_hull_melkman(InputIterator begin, InputIterator end,
                      OutputIterator out) {
    const std::size_t point_count = std::distance(begin, end);
    ...
    if (orientation((*(begin + 0)), (*(begin + 1)), (*(begin + 2))) ==
        LeftHandSide) {
    ...
    for (InputIterator it = (begin + 3); it != end; ++it) {
      if ((orientation(deq[deq.size() - 1], deq[deq.size() - 2], (*it)) ==
           LeftHandSide) &&
          (orientation(deq[1], deq[0], (*it)) == LeftHandSide)) {
        continue;
      }

      while (orientation(deq[deq.size() - 1], deq[deq.size() - 2], (*it)) !=
             LeftHandSide) {
        deq.pop_back();
      }
