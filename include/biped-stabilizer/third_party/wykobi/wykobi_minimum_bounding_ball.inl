...
struct randomized_minimum_bounding_ball<point2d<T> > {
 public:
  ...
  randomized_minimum_bounding_ball(InputIterator begin, InputIterator end,
                                   circle<T>& circle) {
    ...
    switch (point_count) {
      ...
      case 1:
        circle = make_circle(*begin, T(0.0));
        return;

      case 2:
        circle = make_circle(*begin, *(begin + 1));
        return;

      case 3:
        circle = make_circle(*begin, *(begin + 1), *(begin + 2));
        return;
    }
    ...
    for (InputIterator it = point_list.begin() + 2; it != point_list.end();
         ++it) {
...
struct randomized_minimum_bounding_ball_with_ch_filter<point2d<T> > {
 public:
  ...
  randomized_minimum_bounding_ball_with_ch_filter(InputIterator begin,
    ...
    convex_hull_graham_scan<point2d<T> >(begin, end,
                                         std::back_inserter(convex_hull));

    randomized_minimum_bounding_ball<point2d<T> >(convex_hull.begin(),
                                                  convex_hull.end(), circle);
  }
};
...
struct naive_minimum_bounding_ball<point2d<T> > {
 public:
  template <typename InputIterator>
  naive_minimum_bounding_ball(InputIterator begin, InputIterator end,
                              circle<T>& circle) {
    ...
    for (InputIterator it1 = begin; it1 != end; ++it1) {
      for (InputIterator it2 = it1 + 1; it2 != end; ++it2) {
        for (InputIterator it3 = it2 + 1; it3 != end; ++it3) {
          wykobi::circle<T> current_circle =
              make_circle((*it1), (*it2), (*it3));

          bool contains_all_points = true;

          for (InputIterator n = begin; n != end; ++n) {
            if ((n != it1) && (n != it2) && (n != it3) &&
...
struct naive_minimum_bounding_ball_with_ch_filter<point2d<T> > {
 public:
  ...
  naive_minimum_bounding_ball_with_ch_filter(InputIterator begin,
    ...
    naive_minimum_bounding_ball<point2d<T> >(convex_hull.begin(),
                                             convex_hull.end(), circle);
  }
};
...
struct ritter_minimum_bounding_ball<point2d<T> > {
 public:
  template <typename InputIterator>
  ritter_minimum_bounding_ball(InputIterator begin, InputIterator end,
                               circle<T>& circle) {
    switch (std::distance(begin, end)) {
      case 0:
        return;
    ...
    for (InputIterator it = begin; it != end; ++it) {
      ...
      T lay_dist =
          lay_distance_from_point_to_circle_center(current_point, circle);
...
struct ritter_minimum_bounding_ball_with_ch_filter<point2d<T> > {
 public:
  ...
  ritter_minimum_bounding_ball_with_ch_filter(InputIterator begin,
    ...
    ritter_minimum_bounding_ball<point2d<T> >(convex_hull.begin(),
                                              convex_hull.end(), circle);
  }
};
 ...
 public:
  ...
  ritter_minimum_bounding_ball(InputIterator begin, InputIterator end,
    ...
    switch (std::distance(begin, end)) {
      ...
      case 1:
        sphere = make_sphere(*begin, T(0.0));
        return;

      case 2:
        sphere = make_sphere(*begin, *(begin + 1));
        return;
    }
    ...
    for (InputIterator it = begin; it != end; ++it) {
      ...
      T lay_dist =
          lay_distance_from_point_to_sphere_center(current_point, sphere);
