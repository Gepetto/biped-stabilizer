...
struct generate_axis_projection_descriptor {
 public:
  ...
  generate_axis_projection_descriptor(const polygon<T, 2>& polygon,
                                      OutputIterator descriptor) {
    ...
    for (std::size_t i = 0; i < axis_count; ++i) {
      value.push_back(distance(
          project_onto_axis(polygon, make_line(origin_point, rotated_point))));
    ...
    for (typename std::vector<T>::iterator it = value.begin() + 1;
         it != value.end(); ++it) {
