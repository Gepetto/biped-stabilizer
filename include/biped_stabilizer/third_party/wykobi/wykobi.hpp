...
class point2d : public geometric_entity {
 public:
  ...
  inline reference operator()(const std::size_t& index) {
    return ((0 == index) ? x : y);
  }
...
class polygon : public geometric_entity {
 ...
 public:
  ...
  inline void erase(const std::size_t index) {
...
/************[      Sphere Type      ]************/
template <typename T>
class sphere : public geometric_entity {
 public:
  T x, y, z, radius;
};
...
class define_bezier_type<T, 2, eQuadraticBezier> {
 public:
  typedef quadratic_bezier<T, 2> BezierType;
};

template <typename T>
class define_bezier_type<T, 3, eQuadraticBezier> {
 public:
  typedef quadratic_bezier<T, 3> BezierType;
};

template <typename T>
class define_bezier_type<T, 2, eCubicBezier> {
 public:
  typedef cubic_bezier<T, 2> BezierType;
};

template <typename T>
class define_bezier_type<T, 3, eCubicBezier> {
 public:
  typedef cubic_bezier<T, 3> BezierType;
};
...
class vectornd : public pointnd<T, D> {
 public:
  vectornd() { pointnd<T, D>::clear(); }

  vectornd(const T& v0) { pointnd<T, D>::v[0] = v0; }
  ...
  vectornd(const vectornd<T, D>& vec) : pointnd<T, D>() {
...
inline bool collinear(const T& x1, const T& y1, const T& z1, const T& x2,
                      const T& y2, const T& z2, const T& x3, const T& y3,
                      const T& z3, const T& epsilon = T(Epsilon));
...
inline bool is_point_collinear(const point2d<T>& point1,
                               ...
                               const T& py, const bool robust = false);
template <typename T>
...
inline void intersection_point_line_to_line(
    ...
    const T& y4, const T& z4, T& Ix, T& Iy, T& Iz, const T& fuzzy = T(0.0));
...
template <typename T>
inline triangle<T, 2> create_antipedal_triangle(const point2d<T>& point,
                                                const triangle<T, 2>& triangle);
...
template <typename T>
inline point2d<T> closest_point_on_circle_from_circle(const circle<T>& circle1,
                                                      const circle<T>& circle2);

template <typename T>
inline point3d<T> closest_point_on_sphere_from_sphere(const sphere<T>& sphere1,
                                                      const sphere<T>& sphere2);
...
inline rectangle<T> nonsymmetric_mirror(const rectangle<T>& rectangle,
                                        const T& ratio, const line<T, 2>& line);
template <typename T>
...
inline quadix<T, 3> nonsymmetric_mirror(const quadix<T, 3>& quadix,
                                        ...
                                        const plane<T, 3>& plane);
template <typename T>
inline circle<T> nonsymmetric_mirror(const sphere<T>& sphere, const T& ratio,
                                     const plane<T, 3>& plane);
template <typename T>
...
template <typename T>
inline polygon<T, 2> minkowski_sum(const circle<T>& triangle,
                                   const circle<T>& circle);
...
inline polygon<T, 2> minkowski_difference(const quadix<T, 2>& quadix1,
                                          const quadix<T, 2>& quadix2);
template <typename T>
inline polygon<T, 2> minkowski_difference(const circle<T>& triangle,
                                          const circle<T>& circle);
